...
from cgcnn.data_2 import CIFData, collate_pool, get_train_val_test_loader
...
def main():
    ...
    test_idx = list(range(Ntr+Nv,Ntr+2*Nv))
    ...
    n_h = 4
    lr = 0.001
    lr_decay_rate = 0.9962162453911833
    weight_decay = 0
    resume = False
   ...
   # dataset = CIFData(*args.data_options)
    dataset = CIFData(root_dir,fname)
...
#        if args.task == 'regression':
        ...
        torch.save({'epoch': epoch,
                       ...
                       'normalizer': normalizer.state_dict()}, chk_name)
        if is_best:
...
'''
def save_checkpoint(state, is_best, filename=chk_name):
    torch.save(state, filename)
    if is_best:
